...
BinaryDecoder.prototype = {
    ...
    decodeUint32Interleaved( size, offset, byteSize, numElements ) {
        var array = new Uint32Array( size * numElements );
    ...
    decodeUint8Interleaved( size, offset, byteSize, numElements ) {
        var array = new Uint8Array( size * numElements );
        for ( var i = 0; i < size; i++ ) {
            this._offset = offset + i * byteSize;
            for ( var j = 0; j < numElements; j++ ) {
